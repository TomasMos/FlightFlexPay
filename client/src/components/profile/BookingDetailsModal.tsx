...
import { formatTime, formatDuration, formatDate, toTitleCase, stopoverDuration } from '@/utils/formatters';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center z-50 p-4">
      <div className="bg-white w-full max-w-4xl rounded-xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex justify-between items-center p-6 border-b">
          <div>
            <h2 className="text-2xl font-bold" data-testid="text-booking-details-title">
              Booking #{booking.id}
            </h2>
            <p className="text-gray-600">Flight Details & Payment Information</p>
          </div>
          <Button
            variant="ghost"
            size="sm"
                  ...
                  <div key={index} className="border rounded-lg p-4">
                      ...
                      <div className="flex items-center gap-4">
                        <div>
                          ...
                          <div className="text-sm text-gray-600">
                            {index === 0 ? 'Outbound' : 'Return'} • {formatDate(firstSegment.departure.at)}
                          </div>
                        </div>
                      </div>
                      <Button
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-4">
                      <div>
                        ...
                        <div className="text-sm">{firstSegment.departure.iataCode}</div>
                      </div>
                      <div className="text-center">
                        <div className="text-sm text-gray-500">Duration</div>
                        <div className="font-medium">{formatDuration(itinerary.duration)}</div>
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-sm">{lastSegment.arrival.iataCode}</div>
                      </div>
                    </div>
                                ...
                                <div className="bg-gray-50 rounded-lg p-4">
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                      ...
                                      <div className="text-sm space-y-1">
                                        <p>{toTitleCase(segment.departure.airportName)} ({segment.departure.iataCode})</p>
                                        {segment.departure.terminal && <p>Terminal {segment.departure.terminal}</p>}
                                        <p>{toTitleCase(segment.airline)} - {segment.number}</p>
                                    ...
                                    <div>
                                      ...
                                      <div className="text-sm space-y-1">
                                        <p>{toTitleCase(segment.arrival.airportName)} ({segment.arrival.iataCode})</p>
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        {segment.includedCabinBags?.weight && (
                                          <div className="flex items-center gap-2">
                                            ...
                                            <span>
                                              ...
                                              {segment.includedCabinBags.quantity && ` x${segment.includedCabinBags.quantity}`} carry-on
                                            </span>
                                          </div>
                                        )}
                                        {segment.includedCheckedBags?.weight && (
                                          <div className="flex items-center gap-2">
                                            ...
                                            <span>
                                              ...
                                              {segment.includedCheckedBags.quantity && ` x${segment.includedCheckedBags.quantity}`} checked
                                            </span>
                                          </div>
                                        )}
                                      </div>
                                    ...
                                    <div>
                                      ...
                                      <div className="space-y-1 text-sm">
                                        <div className="flex items-center gap-2">
                                          {flightData.pricingOptions?.refundableFare ? (
                                            <>
                                              <Check className="w-4 h-4 text-green-600" />
                                        ...
                                        <div className="flex items-center gap-2">
                                          {flightData.pricingOptions?.noPenaltyFare ? (
                                            <>
                                              <Check className="w-4 h-4 text-green-600" />
                                            ...
                                            <>
                                              ...
                                              <span className="text-yellow-600">Changes with fee</span>
                                            </>
                                          )}
                                        </div>
                                      </div>
                                  ...
                                  <div className="flex items-center justify-center my-4">
                                    <div className="flex items-center gap-2 px-3 py-1 bg-gray-100 rounded-full">
                                      ...
                                      <span className="text-xs text-gray-600">
                                        {toTitleCase(itinerary.segments[segmentIndex + 1].departure.airportName)} 
                                        ({itinerary.segments[segmentIndex + 1].departure.iataCode}) - 
                                        {stopoverDuration(
                                          new Date(segment.arrival.at),
                                          new Date(itinerary.segments[segmentIndex + 1].departure.at)
                                        )}
                                      </span>
                                    </div>
                                  </div>
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-bold">
                  {currencySymbol}{parseFloat(booking.totalPrice).toFixed(2)} {booking.currency}
                </span>
              </div>
                ...
                <>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium">
                      {currencySymbol}{parseFloat(booking.paymentPlan.depositAmount).toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Installment Amounts</span>
                    <span className="font-medium">
                      {currencySymbol}{((parseFloat(booking.paymentPlan.totalAmount) - parseFloat(booking.paymentPlan.depositAmount)) / booking.paymentPlan.installmentCount).toFixed(2)} 
          ...
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <p className="text-gray-600 mb-4">
                Your flight tickets will be available here once generated by the airline.
              </p>
              <Button variant="outline" disabled data-testid="button-tickets-placeholder">
                Tickets (Coming Soon)
              </Button>
            </CardContent>
          </Card>
