...
export function BillingTab() {
  ...
  const { data: paymentPlans = [], isLoading: loadingPlans } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/billing/payment-plans?email=${encodeURIComponent(currentUser?.email || "")}`,
      );
  ...
  const { data: paymentMethods = [], isLoading: loadingMethods } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/billing/payment-methods?email=${encodeURIComponent(currentUser?.email || "")}`,
      );
  ...
  const { data: paymentHistory = [], isLoading: loadingHistory } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/billing/payment-history?email=${encodeURIComponent(currentUser?.email || "")}`,
      );
            ...
            {loadingPlans ? (
              ...
              <Card>
                <CardContent className="p-8 text-center">
                  ...
                  <CardDescription>
                    Your payment plans will appear here when you book flights
              ...
              (paymentPlans as any[]).map((plan: any) => (
                <Card
                  key={plan.id}
                  className="mb-4"
                  data-testid={`card-payment-plan-${plan.id}`}
                >
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <div
                          ...
                          data-testid={`text-booking-number-${plan.bookingId}`}
                        >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                      <div>
                        ...
                        <div className="font-medium">
                          {currencySymbol}
                          {parseFloat(plan.totalAmount).toFixed(2)}
                        </div>
                      </div>
                      <div>
